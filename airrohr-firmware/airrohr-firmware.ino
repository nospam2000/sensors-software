...
String result_PPD, result_SDS, result_PMS, result_HPM;
	...
	// TODO: check if this is ok for powersave mode
	/*
...
/* TODO: find another way for a timer to check OTA updates
...
static void PMS_cmd(PmSensorCmd cmd) {
	...
	case PmSensorCmd::ContinuousMode:
		...
		break;
	case PmSensorCmd::QueryData:
		assert(false && "not supported by this sensor"); // TODO: implement command for powersave mode
		break;
	}
	serialSDS.write(buf, cmd_len);
}
...
void setup_network() {
	...
	if(!sntp_time_set) {
		...
		debug_outln(String(sntp_time_set ? "" : "not ") + F("received"), DEBUG_MIN_INFO);
	}
